(function () {
  ...
  const LABEL_SYNONYMS = {
    ...
    driver: ["driver"],
    jobNumber: ["job number", "job no", "job #", "job id"],
    containerNumber: ["container number", "container no", "container #"],
    storageSite: ["storage site", "warehouse", "depot"],
    inventoryType: ["inventory type", "inv type", "inventory"],
    date: ["date"]
  };

  const LABEL_KEY_BY_NORMALISED = (function () {
    const map = {};
    for (const key in LABEL_SYNONYMS) {
      if (!Object.prototype.hasOwnProperty.call(LABEL_SYNONYMS, key)) continue;
      LABEL_SYNONYMS[key].forEach(function (label) {
  ...
  function matchCanonicalLabel(str) {
    const n = normalizeLabel(str);
    if (!n) return null;
    return LABEL_KEY_BY_NORMALISED[n] || null;
  }

  function isLabelOnlyLine(line) {
    const n = normalizeLabel(line);
    if (!n) return false;
    return !!LABEL_SET[n] || !!LABEL_KEY_BY_NORMALISED[n];
  }
  ...
  function extractLabelValuePairsFromText(text) {
    const pairs = {};
    if (!text) return pairs;

    const rawLines = text.split(/\r?\n/).map(function (l) { return l.trim(); });
    const lines = rawLines.filter(function (l) { return !!l; });

    // Pattern A: "Label: Value" / "Label - Value" / "Label = Value" / "Label / Value"
    lines.forEach(function (line) {
      ...
      if (match) {
        ...
        var canonical = matchCanonicalLabel(left);
        if (canonical && right) {
          key = canonical;
          value = right;
        }
      ...
      if (!key) {
        ...
        if (slashIdx !== -1) {
          var leftS = line.slice(0, slashIdx).trim();
          var rightS = line.slice(slashIdx + 1).trim();
          var canonicalS = matchCanonicalLabel(leftS);
          if (canonicalS && rightS) {
            key = canonicalS;
            value = rightS;
          }
        }
    ...
    // Pattern B: "Label" on one line, value on the next line
    for (var i = 0; i < lines.length - 1; i++) {
      var maybeLabel = lines[i];
      var maybeValue = lines[i + 1];
      var canonical = matchCanonicalLabel(maybeLabel);
      if (canonical && maybeValue && !isLabelOnlyLine(maybeValue) && !pairs[canonical]) {
        pairs[canonical] = maybeValue;
      }
    }
...
`;
  ...
  function buildMappings(forms, text, vision) {
    ...
    const labelPairs = extractLabelValuePairsFromText(text || "");
    const mappings = [];
    ...
    const visibleInputs = allInputs.filter(function (el) {
      if (isFieldVisible(el)) return true;
      if (!el.getAttribute) return false;
      const typeAttr = (el.getAttribute("type") || "").toLowerCase();
      const hasTypeHint = el.getAttribute("data-appycrew-type");
      if (typeAttr === "hidden" && hasTypeHint) return true;
      return false;
    });
    ...
    function findLabelPairForField(meta) {
      if (!labelPairs) return null;
      if (!meta) return null;
      const candidates = [
        meta.typeHint || "",
        meta.label || "",
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        if (!txt) continue;
        const canonical = matchCanonicalLabel(txt);
        if (canonical && labelPairs[canonical]) {
          return labelPairs[canonical];
        }
      }
      return null;
    }
    ...
    function candidateValue(meta, elType, el) {
      ...
      const fromLabelPairs = findLabelPairForField(meta);
      if (fromLabelPairs) {
        return fromLabelPairs;
      }
    ...
    visibleInputs.forEach(function (el) {
      ...
      const isBridgeHidden =
        elType === "hidden" && meta.typeHint && meta.typeHint.length > 0;
