...
async function callVision(imageBase64, textFromOcr) {
  ...
  if (VISION_PROVIDER === "gemini" && hasGemini) return await callGeminiVision(imageBase64);
  if (VISION_PROVIDER === "openai" && hasOpenAI) return await callOpenAIVision(imageBase64);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, error: "Server error while running OCR / AI" });
  }
};
