...
// Simple shared directories for deciding if AI is needed
...
function shouldCallVision(text) {
  ...
  if (!text || !text.trim()) {
    // No OCR text at all → AI is useful if enabled
    return true;
  }
  if (VISION_MODE === "always") return true;

  // fallback mode: only call AI when OCR + directories are not obviously enough
  const lower = text.toLowerCase();

  const hasItem = ITEM_KEYWORDS.some((k) => lower.indexOf(k) !== -1);
  const hasLocation = LOCATION_KEYWORDS.some((k) => lower.indexOf(k) !== -1);

  // If we already see a clear item or location word, OCR is probably enough
  if (hasItem || hasLocation) return false;

  // If text is extremely short, AI might still help
  if (text.trim().length < 10) return true;

  // Default: don't call AI
  return false;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // AI-second (optional, based on VISION_MODE & directories)
    ...
    res.status(500).json({
      success: false,
      error: "Server error while running OCR / AI"
    });
  }
};
