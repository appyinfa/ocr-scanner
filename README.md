# AppyCrew OCR + Vision Widget (v10)

v10 builds on v9 with better job-sheet extraction and stronger field mapping:

- **Job sheet label → value mapping**
  - New label parser understands patterns like:
    - `Surveyor: Molly Wolly`
    - `Client / Lux Designers Ltd`
    - `Job Number` on one line, value on the next.
  - Recognised labels (canonical keys):
    - `surveyor`, `client`, `driver`, `jobNumber`, `containerNumber`, `storageSite`, `inventoryType`, `date`.
  - The widget now uses these pairs first when filling matching fields.

- **Support for hidden “bridge” inputs**
  - If your React selects / radios sync to a hidden `<input type="hidden" data-appycrew-type="...">`,
    the widget will now treat those as fillable targets even though they’re not visible.

- **Item synonym bugfix**
  - Fixed a typo in the item synonym matcher so synonyms like `carton → box` are always picked up.

Everything else from v9 (select/radio best-match filling, React-friendly updates, description clean-up, SPA detection) still works the same.
